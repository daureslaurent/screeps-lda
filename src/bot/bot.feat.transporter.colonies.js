...
class BotFeatTransporterColonies extends _moduleBot {
    /**
     *
    ...
    constructor(gameAccess, roomDataFactory) {
        super('BotFeatTransporterColonies', BotRole.COLONIES_TRANSPORTER, gameAccess, roomDataFactory);
    }
    ...
    run(creepFast) {
        ...
        if (creep.memory.worker !== undefined) {
            ...
            if (creep.memory.worker.runningId !== undefined) {
                const del = this.roomDataFactory.getRoomData(creep.memory.baseRoom)
                    .getSysColoniesWorker().getWorkerRunning().getRunning(creep.memory.worker.runningId)
                if (del === undefined) {
                    creep.memory.idD = undefined;
        ...
        if (!creep.pos.inRangeTo(waitingPos, 1) && creep.memory.idD === undefined) {
            ...
            if (creep.memory.starterWaintingCount > 1) {
                utils.talkAction(creep, '⭕')
                const currentCarry = creep.store.getUsedCapacity(RESOURCE_ENERGY)
                if (currentCarry > 0) {
                    ...
                    if (creep.memory.idD === undefined) {
                        creep.memory.idD = mainStorage.deposit(creep.id, RESOURCE_ENERGY, trx);
                    }

                    if (mainStorage.getAvailable(RESOURCE_ENERGY, trx)) {
...
module.exports = BotFeatTransporterColonies;